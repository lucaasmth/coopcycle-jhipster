...
public interface ClientRepository extends JpaRepository<Client, Long> {
    ...
    @Query(
        value = "select distinct client from Client client left join fetch client.restaurant",
        countQuery = "select count(distinct client) from Client client"
    )
    Page<Client> findAllWithToOneRelationships(Pageable pageable);

    @Query("select distinct client from Client client left join fetch client.restaurant")
    List<Client> findAllWithToOneRelationships();

    @Query("select client from Client client left join fetch client.restaurant where client.id =:id")

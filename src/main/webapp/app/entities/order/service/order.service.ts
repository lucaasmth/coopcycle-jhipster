...
type RestOf<T extends IOrder | NewOrder> = Omit<T, 'orderDate' | 'deliveryDate'> & {
...
export class OrderService {
  ...
  protected convertDateFromClient<T extends IOrder | NewOrder | PartialUpdateOrder>(order: T): RestOf<T> {
    return {
      ...
      deliveryDate: order.deliveryDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restOrder: RestOrder): IOrder {
    return {
      ...
      deliveryDate: restOrder.deliveryDate ? dayjs(restOrder.deliveryDate) : undefined,
    };
  }
